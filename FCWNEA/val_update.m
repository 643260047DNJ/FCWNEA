...
%   此处显示详细说明
    ...
    for i =1:length(mm)
        ...
        if tl~=length(val_net{mm(i)})
            ind_state = [ind_state;zeros(length(val_net{mm(i)})-tl,3)];
        end
        ...
        if rr(end)>9
            temp_r(j-rr(end)+1:length(val_net{mm(i)}))=1;
        end   
        R = rr.*temp_r;
...
%         end
        ...
        for j =1:length(R)-1
            if (R(j)~=0 & R(j+1)==0 ) || ( R(j+1)~=0 && j==length(R)-1)
                    mark =1;
                    if R(j+1)~=0
                        nn=round(length(val_net{mm(i)})*R(j+1)./sum(R~=0));
                    else
                        nn=round(length(val_net{mm(i)})*R(j)./sum(R~=0));
                    end
        ...
        if length(new_vnet) ~=length(val_net{mm(i)})
            if length(new_vnet)<length(val_net{mm(i)})
                while length(new_vnet)~=length(val_net{mm(i)})
                    if length(new_vnet)<2
                        a=0;
                    end
                    mark1 = randperm(length(new_vnet),1);
