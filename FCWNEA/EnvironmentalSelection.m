function [Pop,Crowdis,Ind_Dec,FFrontNo,N_Gra,w_net,lif_net] = EnvironmentalSelection(Population,N,Z,Zmin,Ind_dec,w_net,Gra,lif_net,phi)
...
%                     w_net{j(k)} = w_net{j(k)}./2+ normpdf([1:51],m1(j(k)),normv)';               
                    w_net{j(k)} = w_net{j(k)}./10+ normpdf([1:length(w_net{j(k)})],m2(j(k)),normv)';     
                else
%                     w_net{j(k)}(m1(j(k)-1),:) = w_net{j(k)}(m1(j(k)-1),:)./2+ normpdf([1:51],m1(j(k)),normv);          
                    w_net{j(k)}(m2(j(k)-1),:) = w_net{j(k)}(m2(j(k)-1),:)./10+ normpdf([1:length( w_net{j(k)}(m2(j(k)-1),:))],m2(j(k)),normv); 
%                      w_net{j(k)}(m1(j(k)-1),:) =  normpdf([1:length( w_net{j(k)}(m1(j(k)-1),:))],m1(j(k)),normv).^2;  
                end
                %更新变量活跃度
                if m_dom==3
                    D_state(1,j(k)) = D_state(1,j(k))+10;
                else
                    if lif_net(j(k)) >100
...
%                     w_net{(k)} = w_net{(k)}./2+ normpdf([1:51],m1(j(k)),normv)';
                    w_net{(k)} = w_net{(k)}./1+ normpdf([1:length(w_net{j(k)})],m2(j(k)),normv)';
                else
%                     w_net{j(k)}(m1(j(k)-1),:) = w_net{j(k)}(m1(j(k)-1),:)./2+ normpdf([1:51],m1(j(k)),normv);
                    w_net{j(k)}(m2(j(k)-1),:) = w_net{j(k)}(m2(j(k)-1),:)./10+ normpdf([1:length( w_net{j(k)}(m2(j(k)-1),:))],m2(j(k)),normv);
%                      w_net{j(k)}(m1(j(k)-1),:) =  normpdf([1:length( w_net{j(k)}(m1(j(k)-1),:))],m1(j(k)),normv).^2;
