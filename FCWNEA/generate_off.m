...
function [Population,Ind_dec,Off,Off_ind,Gra,act_net]=generate_off(Population,Ind_dec,val_net,w_net,act_net,lif_net,N,alph)
    %% 
...
%     S = randperm(N,N);
    ...
    tau = sin(8*pi.*(alph-1))/20+exp((alph-1));
    F2 = 2*floor(0.4*N*tau/2)
    F1 = round(tau * (N-F2));
%     alph=alph/4+0.5;     
%     F2 = 2*ceil(50*alph/2);
...
%    sel_var = randperm(length(vitality),2);
   for i =1: F1
       parent = Ind_dec(S(i),:);
       % 选择要调整的变量
...
%        sel_var = randperm(length(vitality),1);
       for k=1:length(sel_var)
           ...
           if sel_var(k)==1
            ...
            % 生成变量时临时调整权重，父个体选择值域权重调为0，防止子代个体搜索相同区域
            if parent(sel_var(k))< length(Pro)
    %             Pro(parent(sel_var)+1)= Pro(sel_var);
                Pro(parent(sel_var(k))+1)= Pro(parent(sel_var(k))+1)+ max(Pro);
            end
            if parent(sel_var(k))>1
    %             Pro(parent(sel_var)-1) = Pro(sel_var);
                Pro(parent(sel_var(k))-1) = Pro(parent(sel_var(k))-1)+ max(Pro);
            end
...
%        sel_var = select(1+ max(ss)-ss);   
   ...
   %% 拓展延伸  ： 选取两个拥挤度较小的解，在两解的连线之间拓展生成新的解
   for i=N-F2+1:2:N
       parent1 = Ind_dec(S(i),:);
       parent2 = Ind_dec(S(i+1),:);
       del = parent2-parent1;
       
       u = 2*rand(1,2)-1;
       off1_ind = round(parent1+del*u(1));
       off2_ind = round(parent2-del*u(2));
       
%        off1_ind = round(parent1-del*0.5);
%        off2_ind = round(parent2+del*2);
       
       %越界修正
       off1_ind(off1_ind<1)=1;
       off1_ind(off1_ind>51)=51;
%        off1_ind(off1_ind(1:end-3)>51)=51;
%        off1_ind([zeros(1,length(off1_ind)-3)>1,off1_ind(end-2:end)>10])=10;
       off2_ind(off2_ind<1)=1;
       off2_ind(off2_ind>51)=51;
%        off2_ind(off2_ind(1:end-3)>51)=51;
%        off2_ind([zeros(1,length(off2_ind)-3)>1,off2_ind(end-2:end)>10])=10;
       
       Off_ind(S(i),:) = off1_ind;
       Off_ind(S(i+1),:) = off2_ind;
       dec1 = indtodec(off1_ind,val_net);
       dec2 = indtodec(off2_ind,val_net);
       Off_dec(S(i),:) = dec1;
       Off_dec(S(i+1),:) = dec2;
       ...
       %更新变量值域访问量 
        ...
        for j=1:length(S1)
            if j==1
                act_net{j}(S1(j) )= act_net{1}(S1(j) ) +1;
                act_net{j}(S2(j) )= act_net{1}(S2(j) ) +1;
            else
                act_net{j}(S1(j-1),S1(j)) = act_net{j}(S1(j-1),S1(j))+1;
                act_net{j}(S2(j-1),S2(j)) = act_net{j}(SS(j-1),S2(j))+1;
            end
        end
